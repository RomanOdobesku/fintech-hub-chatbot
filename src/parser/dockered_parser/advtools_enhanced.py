...
def _sitemaps_from_robotstxt(robots_url, timeout=None):
    sitemaps = []
    robots_page = urlopen(Request(robots_url, headers=headers), timeout=timeout)
...
def _sitemap_wrapper(url, timeout=None):
    return sitemap_to_df(sitemap_url=url, timeout=timeout)


def sitemap_to_df(sitemap_url, max_workers=8, recursive=True, timeout=None):
    """
    ...
    if sitemap_url.endswith("robots.txt"):
        return pd.concat(
            [
                ...
                for sitemap in _sitemaps_from_robotstxt(sitemap_url, timeout=timeout)
            ],
            ignore_index=True,
        )
    if sitemap_url.endswith("xml.gz"):
        ...
        xml_text = urlopen(Request(sitemap_url, headers=headers), timeout=timeout)
        try:
            resp_headers = xml_text.getheaders()
        except AttributeError:
            resp_headers = ""
    ...
    if (root.tag.split("}")[-1] == "sitemapindex") and recursive:
        ...
        with futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for sitemap in sitemap_url_list:
                future = executor.submit(_sitemap_wrapper, sitemap, timeout)
                to_do.append(future)
