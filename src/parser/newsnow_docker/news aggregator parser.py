...
def proxy_requests_aggregator_url(url: str, proxies: Dict[str, str]) -> List[Dict[str, str]]:
    ...
    for news in html_news:
        try:
            ...
            content_news_for_one_url.append(
                {"href": news_href, "title": news_title, "publisher": news_publisher, "time": news_time,
...
def news_aggregator_parser(check_minutes: int = 60, start_proxy_index_list_url: int = 0,
                           start_proxy_index_list_redirect: int = 0) -> List[Dict[str, str]]:
    """
    ...
    for i in range(len(filtered_content_news)):
        while number_proxy_error_consistently == 0 \
            ...
            list_news.append(
                {"url": news_url, "title": filtered_content_news[i]["title"],
                 "publisher": filtered_content_news[i]["publisher"],
...
def list_json_to_df(links: list[dict]) -> pd.DataFrame:
    if len(links) == 0:
        return pd.DataFrame(columns=['url', 'title', 'content', 'time'])
    return pd.DataFrame(links).drop(columns=['article_time_str', 'article_title_generate', 'article_publisher']).rename(
        columns={'article_url': 'url', 'article_title': 'title', 'article_text': 'content', 'article_time': 'time'})
